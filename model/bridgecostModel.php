...
Class bridgecostModel Extends baseModel {
	protected $table = "bridge_cost";
    ...
    public function deleteBridgecost($id){
    	if ($this->getBridgecost($id)) {
    		return $this->delete($this->table,array('bridge_cost_id'=>$id));
    	}
    }
    ...
    public function getAllBridgecostByWhere($id){
        return $this->query('SELECT * FROM bridge_cost WHERE bridge_cost_id != '.$id);
    }
