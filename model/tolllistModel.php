...
Class tolllistModel Extends baseModel {
	protected $table = "toll_list";
    ...
    public function deleteToll($id){
    	if ($this->getToll($id)) {
    		return $this->delete($this->table,array('toll_list_id'=>$id));
    	}
    }
    ...
    public function getAllTollByWhere($id){
        return $this->query('SELECT * FROM toll_list WHERE toll_list_id != '.$id);
    }
    public function getLastToll(){
        return $this->getLast($this->table);
    }
    public function checkToll($id,$toll_name){
        return $this->query('SELECT * FROM toll_list WHERE toll_list_id != '.$id.' AND toll_list_name = '.$toll_name);
    }
