...
Class roadcostController Extends baseController {
    ...
    public function index() {
        ...
        $this->view->data['title'] = 'Phí sử dụng đường bộ';
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            $data = array(
                        ...
                        'road_cost_date'=>strtotime($_POST['road_cost_date']),

                        'road_cost_number' => trim($_POST['road_cost_number']),

                        'road_cost_vat' => trim(str_replace(',', '', $_POST['road_cost_vat'])),

                        'road_cost_price' => trim(str_replace(',', '', $_POST['road_cost_price'])),
            ...
            if ($_POST['yes'] != "") {
                ...
                if ($road_cost_model->checkCost($_POST['yes'].' AND road_cost_number = "'.trim($_POST['road_cost_number']).'"')) {
                    ...
                    $road = $road_cost_model->getCost($_POST['yes']);

                    $road_cost_model->updateCost($data,array('road_cost_id' => $_POST['yes']));

                    $id_road_cost = $_POST['yes'];
                    ...
                    $data_debit = array(
                        ...
                        'debit_date'=>$data['road_cost_date'],

                        'customer'=>$data['customer'],

                        'money'=>$data['road_cost_price'],

                        'money_vat_price'=>$data['road_cost_vat'],

                        'comment'=>$data['road_cost_comment'].' '.$data['road_cost_number'],

                        'check_debit'=>2,

                        'road_cost'=>$id_road_cost,

                    );

                    $debit_model->updateDebit($data_debit,array('road_cost'=>$id_road_cost));


                    if ($road->road_cost_vat > 0 && $data['road_cost_vat'] > 0) {
                        $data_vat = array(

                            'in_out'=>1,

                            'vat_number'=>$data['road_cost_number'],

                            'vat_date'=>$data['road_cost_date'],

                            'road_cost'=>$id_road_cost,

                            'vat_sum'=>$data['road_cost_price'],

                            'vat_price'=>$data['road_cost_vat'],

                        );

                        $vat_model->updateVAT($data_vat,array('road_cost'=>$id_road_cost));
                    }
                    else if (($road->road_cost_vat == 0 || $road->road_cost_vat == "") && $data['road_cost_vat'] > 0) {
                        $data_vat = array(
                    ...
                    else if ($road->road_cost_vat > 0 && ($data['road_cost_vat'] == 0 || $data['road_cost_vat'] == "")) {
                        $vat_model->queryVAT('DELETE FROM vat WHERE road_cost = '.$id_road_cost);
                    }
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."edit"."|".$_POST['yes']."|road_cost|".implode("-",$data)."\n"."\r\n";
                ...
                if ($road_cost_model->getCostByWhere(array('road_cost_number'=>$data['road_cost_number']))) {
                    ...
                    $road_cost_model->createCost($data);

                    $id_road_cost = $road_cost_model->getLastCost()->road_cost_id;
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."add"."|".$road_cost_model->getLastCost()->road_cost_id."|road_cost|".implode("-",$data)."\n"."\r\n";
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['xoa'])) {
                ...
                foreach ($data as $data) {
                    ...
                    $road_cost = $road_cost_model->getCost($data);


                    $vat_model->queryVAT('DELETE FROM vat WHERE road_cost = '.$data);

                    $debit_model->queryDebit('DELETE FROM debit WHERE road_cost = '.$data);


                    $road_cost_model->deleteCost($data);
                ...
                $road_cost = $road_cost_model->getCost($_POST['data']);


                $vat_model->queryVAT('DELETE FROM vat WHERE road_cost = '.$_POST['data']);

                $debit_model->queryDebit('DELETE FROM debit WHERE road_cost = '.$_POST['data']);
                ...
                return $road_cost_model->deleteCost($_POST['data']);
