...
Class paymentvoucherController Extends baseController {
    ...
    public function index() {
        ...
        foreach ($tolls as $toll) {
                ...
                $toll_data[$toll->toll_id]['id'] = $toll->toll_id;

                $toll_data[$toll->toll_id]['name'] = $toll->toll_name;

        }

        $this->view->data['data_toll'] = $toll_data;
        ...
        if ($keyword != '') {
            ...
            $search = '(
                    ...
                    OR toll IN (SELECT toll_id FROM toll WHERE toll_name LIKE "%'.$keyword.'%")
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($_POST['type'] == 1) {
                ...
                foreach ($list as $rs) {
                    ...
                    echo '<li onclick="set_item_customer(\''.$rs->toll_id.'\',\''.$rs->toll_name.'\')">'.$toll_name.'</li>';
    ...
    public function getDebit(){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($_POST['type'] == 1) {
                ...
                $data = array(
                    'where' => 'check_debit = 2 AND staff = '.$_POST['customer'],
                );
                ...
                $data = array(
                    'where' => 'check_debit = 2 AND steersman = '.$_POST['customer'],
                );
                ...
                $data = array(
                    'where' => 'check_debit = 2 AND toll = '.$_POST['customer'],
                );
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            else if(trim($_POST['type']) == 4){
                $data['toll'] = trim($_POST['customer']);
            }
