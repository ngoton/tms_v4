...
Class payableController Extends baseController {
    public function index() {
        ...
        if (!isset(json_decode($_SESSION['user_permission_action'])->payable) || json_decode($_SESSION['user_permission_action'])->payable != "payable") {
            $this->view->data['disable_control'] = 1;
        }
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Công nợ phải trả';
        ...
        $data = array(
            'where'=>'check_debit = 2 AND debit_date < '.strtotime($batdau),
        );
        ...
        foreach ($debit_olds as $debit) {
            ...
            else if ($debit->toll > 0) {
                $data_old['toll'][$debit->toll] = isset($data_old['toll'][$debit->toll])?$data_old['toll'][$debit->toll]+($debit->money+$debit->money_vat_price-$pay_money):($debit->money+$debit->money_vat_price-$pay_money);
            }
        }
        ...
        foreach ($debit_news as $debit) {
            ...
            if ($debit->customer > 0) {
                ...
                $data_new['toll'][$debit->toll]['no'] = isset($data_new['toll'][$debit->toll]['no'])?$data_new['toll'][$debit->toll]['no']+($debit->money+$debit->money_vat_price):($debit->money+$debit->money_vat_price);
                $data_new['toll'][$debit->toll]['co'] = isset($data_new['toll'][$debit->toll]['co'])?$data_new['toll'][$debit->toll]['co']+($pay_money):($pay_money);
            }
        }
        ...
        $tolls = $toll_model->getAllToll(array('order_by'=>'toll_name ASC'));
   ...
   public function customer() {
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Phải trả khách hàng';
        ...
        $data = array(
            'where' => 'check_debit = 2 AND check_loan = 2 AND customer > 0 AND debit_date >= '.strtotime($batdau).' AND debit_date < '.strtotime($ngayketthuc),
        );
        ...
        $this->view->show('payable/customer');
    }
    public function staff() {
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Phải trả nhân viên';
        ...
        $data = array(
            'where' => 'check_debit = 2 AND check_loan = 2 AND staff > 0 AND debit_date >= '.strtotime($batdau).' AND debit_date < '.strtotime($ngayketthuc),
        );
    ...
    public function steersman() {
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Phải trả tài xế';
        ...
        $this->view->show('payable/steersman');
    }
    public function toll() {
        ...
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Phải trả phí cầu đường';
        ...
        $join = array('table'=>'toll','where'=>'debit.toll=toll_id');
        ...
        $data = array(
            'where' => 'check_debit = 2 AND check_loan = 2 AND toll > 0 AND debit_date >= '.strtotime($batdau).' AND debit_date < '.strtotime($ngayketthuc),
        );
        ...
        if($kh > 0){
            $data['where'] = $data['where'].' AND debit.toll = '.$kh;
        }
        ...
        if ($keyword != '') {
            $search = ' AND ( toll_name LIKE "%'.$keyword.'%"  
                        OR debit.comment LIKE "%'.$keyword.'%" 
                        )';
            $data['where'] .= $search;
        }
