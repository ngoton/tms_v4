...
Class checkingcostController Extends baseController {
    ...
    public function index() {
        ...
        if (!isset(json_decode($_SESSION['user_permission_action'])->checkingcost) || json_decode($_SESSION['user_permission_action'])->checkingcost != "checkingcost") {
        ...
        $this->view->data['title'] = 'Phí kiểm định';
        ...
        $data = array(
            ...
            'where' => 'checking_cost_date >= '.strtotime($batdau).' AND checking_cost_date < '.strtotime($ngayketthuc),
        ...
        if (isset($id) && $id > 0) {
            $data['where'] = 'checking_cost_id = '.$id;
        }
        ...
        $tongsodong = count($checking_cost_model->getAllCost($data,$join));
        ...
        if ($keyword != '') {
            ...
            $ngay = (strtotime(str_replace("/", "-", $keyword))!="")?(' OR checking_cost_date LIKE "%'.strtotime(str_replace("/", "-", $keyword)).'%"'):"";
            ...
            $search = '(
                    ...
                    checking_cost_number LIKE "%'.$keyword.'%"

                    OR checking_cost_comment LIKE "%'.$keyword.'%" 
        ...
        $checking_costs = $checking_cost_model->getAllCost($data,$join);



        

        $this->view->data['checking_costs'] = $checking_costs;





        $this->view->data['lastID'] = isset($checking_cost_model->getLastCost()->checking_cost_id)?$checking_cost_model->getLastCost()->checking_cost_id:0;





        $this->view->show('checkingcost/index');
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            $data = array(
                        ...
                        'checking_cost_date'=>strtotime($_POST['checking_cost_date']),

                        'checking_cost_number' => trim($_POST['checking_cost_number']),

                        'checking_cost_vat' => trim(str_replace(',', '', $_POST['checking_cost_vat'])),

                        'checking_cost_price' => trim(str_replace(',', '', $_POST['checking_cost_price'])),
            ...
            if ($_POST['yes'] != "") {
                ...
                if ($checking_cost_model->checkCost($_POST['yes'].' AND checking_cost_number = "'.trim($_POST['checking_cost_number']).'"')) {
                    ...
                    $checking = $checking_cost_model->getCost($_POST['yes']);

                    $checking_cost_model->updateCost($data,array('checking_cost_id' => $_POST['yes']));

                    $id_checking_cost = $_POST['yes'];
                    ...
                    $data_debit = array(
                        ...
                        'debit_date'=>$data['checking_cost_date'],

                        'customer'=>$data['customer'],

                        'money'=>$data['checking_cost_price'],

                        'money_vat_price'=>$data['checking_cost_vat'],

                        'comment'=>$data['checking_cost_comment'].' '.$data['checking_cost_number'],

                        'check_debit'=>2,

                        'checking_cost'=>$id_checking_cost,

                    );

                    $debit_model->updateDebit($data_debit,array('checking_cost'=>$id_checking_cost));


                    if ($checking->checking_cost_vat > 0 && $data['checking_cost_vat'] > 0) {
                        $data_vat = array(

                            'in_out'=>1,

                            'vat_number'=>$data['checking_cost_number'],

                            'vat_date'=>$data['checking_cost_date'],

                            'checking_cost'=>$id_checking_cost,

                            'vat_sum'=>$data['checking_cost_price'],

                            'vat_price'=>$data['checking_cost_vat'],

                        );

                        $vat_model->updateVAT($data_vat,array('checking_cost'=>$id_checking_cost));
                    }
                    else if (($checking->checking_cost_vat == 0 || $checking->checking_cost_vat == "") && $data['checking_cost_vat'] > 0) {
                        $data_vat = array(
                    ...
                    else if ($checking->checking_cost_vat > 0 && ($data['checking_cost_vat'] == 0 || $data['checking_cost_vat'] == "")) {
                        $vat_model->queryVAT('DELETE FROM vat WHERE checking_cost = '.$id_checking_cost);
                    }
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."edit"."|".$_POST['yes']."|checking_cost|".implode("-",$data)."\n"."\r\n";
                ...
                if ($checking_cost_model->getCostByWhere(array('checking_cost_number'=>$data['checking_cost_number']))) {
                    ...
                    $checking_cost_model->createCost($data);

                    $id_checking_cost = $checking_cost_model->getLastCost()->checking_cost_id;
                    ...
                    date_default_timezone_set("Asia/Ho_Chi_Minh"); 
                        ...
                        $text = date('d/m/Y H:i:s')."|".$_SESSION['user_logined']."|"."add"."|".$checking_cost_model->getLastCost()->checking_cost_id."|checking_cost|".implode("-",$data)."\n"."\r\n";
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['xoa'])) {
                ...
                foreach ($data as $data) {
                    ...
                    $checking_cost = $checking_cost_model->getCost($data);
                ...
                $checking_cost = $checking_cost_model->getCost($_POST['data']);
                ...
                return $checking_cost_model->deleteCost($_POST['data']);
